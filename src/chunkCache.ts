...
// tslint:disable-next-line: max-line-length
                logger.logError('Вы только что столкнулись с админом. Он помешал вам разместить пиксель. ОСТАНОВКА СЕЙЧАС!');
                process.exit(1);
                throw new Error('Остановлено администратором');
            default:
                throw new Error(`Пиксельная публикация ответила ${error.message}`);
            }
        });
        ...
        default:
            throw new Error(`Пиксельная публикация ответила ${resp.statusText}`);
        }
    }
...
// tslint:disable-next-line: max-line-length
        logger.log(`Пиксель поставлен: ${chunkX * 256 + pixelIdInChunk % 256 - 32768}:${chunkY * 256 + Math.floor(pixelIdInChunk / 256) - 32768}, цвет: ${color}`);
...
// tslint:disable-next-line: max-line-length
            ...
            case 403:
                    ...
                    // At this point it's better to just stop.
// tslint:disable-next-line: max-line-length
                logger.logError('Вы только что столкнулись с Admin. Он помешал вам разместить пиксель. ОСТАНОВКА СЕЙЧАС!');
                process.exit(1);
                throw new Error('Остановлено администратором');
            default:
                throw new Error(`Сбор чанка ответил ${error.response.statusText}`);
            }
        });
        ...
        case 200:
            ...
            return buffer;
        default:
            throw new Error(`Сбор чанка ответил ${resp.statusText}`);
        }
    }
}
