...
export class WebSocketHandler {
    ...
    private onOpen() {
        ...
        logger.log('Начало прослушивания изменений через веб-сокет');
    ...
    private onClose(event: {wasClean: boolean, code: number, reason: string, target: ws}) {
        this.webSocket = undefined;
        logger.logWarn(`Сокет был закрыт. Восстановление... ${event.reason}`);
        this.connect();
    }

    private onError(event: {error: any, message: string, type: string, target: ws}) {
        logger.logError(`Ошибка, закрытие сокета ${event.message}`);
        event.target.close();
    }
}
