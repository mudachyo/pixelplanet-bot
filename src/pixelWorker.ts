...
export class PixelWorker {
    ...
    constructor(imgProcessor: ImageProcessor,
        ...
        this.initializeData().catch(() => {
            logger.logError('Инициализация не удалась!');
        });
    }
...
// tslint:disable-next-line: max-line-length
                logger.log(`Разместил пиксель ${targetColor} на ${currentTargetCoords!.x}:${currentTargetCoords!.y}`);

                if (postPixelResult.waitSeconds > 50) {
                    const waitingFor = Math.floor(postPixelResult.waitSeconds - Math.random() * 40);

                    logger.log(`Пикселей осталось разместить/проверить: ${this.currentWorkingList.length}`);
                    logger.log(`Ожидаем: ${waitingFor} секунд`);
